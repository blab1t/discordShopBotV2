...
async function handleProxy(interaction) {
  ...
  if (sub === 'create') {
    const specifiedOwner = interaction.options.getUser('owner');
    return proxyWizard.startWizard(interaction, {
      ...
      suppressProxyNotice: !specifiedOwner,
      hideProxyLabel: !specifiedOwner,
    });
  }
  ...
  if (!listing) {
    return interaction.reply({ content: `No listing found for **${ign}**.`, flags: EPH });
  }
  if (sub === 'edit') {
    return interaction.reply({
      content: `Editing **${listings.displayIgn(listing)}**. What do you want to change?`,
  ...
  if (sub === 'delete') {
    ...
    return interaction.reply({
      content: `Delete the listing for **${listings.displayIgn(listing)}**? This removes the published message and its channel.${
...
async function listProxyCategories(interaction) {
  ...
  return interaction.reply({ content: `**Proxy Categories**\n${text}`, flags: EPH });
}
...
async function renameProxyCategory(interaction) {
  ...
  try {
    ...
    return interaction.editReply(
      `Renamed proxy category to **${category.label}**.${discordCategoryChanged ? ' Its Discord category was renamed too.' : ' No managed Discord category exists yet.'}`
    );
  } catch (err) {
...
async function deleteProxyCategory(interaction) {
  ...
  try {
    ...
    return interaction.editReply(`Deleted ${scope} proxy category **${category.label}** from the bot.${discordResult}`);
  } catch (err) {
    return interaction.editReply(err.message);
  }
}
...
async function organizeProxies(interaction) {
  ...
  const parts = [
    `Organized ${result.listingMoves} proxy channel(s) and ${result.ticketMoves} proxy ticket(s).`,
  ...
  logs.ticket(interaction.client, 'proxy organize finished', `${summary} Run by <@${interaction.user.id}>.`);
  const edited = await interaction.editReply(summary).then(() => true).catch(() => false);
  if (!edited) {
    await interaction.channel.send({
      content: `<@${interaction.user.id}> proxy organize finished: ${summary}`,
...
async function transferProxy(interaction) {
  ...
  if (listingChannel.id === ticketChannel.id) {
    return interaction.reply({ content: 'The proxy channel and ticket channel must be different.', flags: EPH });
  }
  ...
  if (!ign || !mojang.isValidIgn(ign)) {
    return interaction.reply({ content: 'Add a valid `ign` because I could not safely derive one from the proxy channel name.', flags: EPH });
  }
...
async function restoreSoldListing(interaction) {
  ...
  logs.listing(interaction.client, 'restored from sold', fresh, interaction.user.id, [
    ...
    { name: 'C/O', value: listings.displayUsdPrice(fresh.co), inline: true },
  ]);
  return interaction.editReply({
    content: `**${listings.displayIgn(fresh)}** is back on the market${channel ? ` in <#${channel.id}>` : ' (its channel is gone - accept it again to create one)'}. BIN **${listings.displayUsdPrice(fresh.bin)}**, C/O **${listings.displayUsdPrice(fresh.co)}**.`,
...
async function checkProxyIgns(interaction) {
  ...
  if (stuck.length) sections.push(`**Stuck half-accepted (${stuck.length})** - fix with \`/proxy publish\`\n${stuck.join('\n')}`);
  if (flagged.length) sections.push(`**Bad usernames (${flagged.length})** - fix with \`/proxy edit\`, use \`/proxy hide\` for privacy\n${flagged.join('\n')}`);
  if (!sections.length) {
...
async function hideProxyIgn(interaction) {
  ...
  if (rename) {
    ...
    if (fresh.ticket_channel_id) {
      ...
      if (ticketChannel) {
        await ticketChannel.setName(tickets.sanitizeChannelName(`${label}-proxy${ticket ? `-${ticket.number}` : ''}`)).catch(() => {});
...
async function reassignProxy(interaction) {
  ...
  if (newOwner.bot) {
    return interaction.reply({ content: 'You cannot transfer a proxy to a bot.', flags: EPH });
  }
  if (String(listing.requester_id) === newOwner.id) {
    return interaction.reply({ content: `<@${newOwner.id}> already owns that proxy.`, flags: EPH, allowedMentions: { parse: [] } });
  }
  ...
  await tickets.sendTicketWelcome(
    channel,
    ticket,
    `This proxy for **${listings.displayIgn(listing)}** was transferred to you by <@${interaction.user.id}>. Staff will coordinate the handover here.`
  );
...
async function handleRecover(interaction) {
  ...
  if (sub === 'dm-owners') {
    ...
    if (dryRun) {
      return interaction.editReply(`Dry run: **${result.total}** proxy owner(s) would be messaged.`);
    }
    logs.send(interaction.client, { title: 'Owners messaged', description: `${result.sent}/${result.total} proxy owners DMed by <@${interaction.user.id}>` });
    return interaction.editReply(`DMed **${result.sent}/${result.total}** proxy owner(s)${result.failed ? ` - ${result.failed} could not be reached (DMs closed or left Discord)` : ''}.`);
  }
  return null;
}
...
async function handleBin(interaction) {
  ...
  if (amount === listing.bin) {
    return interaction.reply({ content: `The BIN of **${listings.displayIgn(listing)}** is already **${listings.displayUsdPrice(amount)}**.`, flags: EPH });
  }
  await interaction.deferReply({ flags: EPH });
  ...
  if (interaction.options.getBoolean('announce') !== false) {
    await listings.announceListingUpdate(interaction.client, updated, `Bin ${verb} to **${listings.displayUsdPrice(amount)}**`);
  }
  await listings.notifyWatchers(interaction.client, updated, `BIN ${verb} to **${listings.displayUsdPrice(amount)}**.`).catch(() => {});
  logs.listing(interaction.client, `BIN ${verb}`, listing, interaction.user.id, [
  ...
  return interaction.editReply(`BIN of **${listings.displayIgn(listing)}** ${verb} to **${listings.displayUsdPrice(amount)}**.`);
}
...
async function handleOffer(interaction) {
  ...
  if (sub === 'list') {
    ...
    if (!offers.length) {
      return interaction.reply({ content: `No offers recorded for **${listings.displayIgn(listing)}**. Current C/O: **${listings.displayUsdPrice(listing.co)}**.`, flags: EPH });
    }
    ...
    return interaction.reply({
      content: `Offers on **${listings.displayIgn(listing)}** (current C/O: **${listings.displayUsdPrice(listing.co)}**):\n${lines.join('\n').slice(0, 1800)}`,
      flags: EPH,
      allowedMentions: { parse: [] },
    });
  }
  ...
  if (sub === 'set') {
    ...
    logs.listing(interaction.client, 'C/O changed', listing, interaction.user.id, [
      { name: 'Old', value: listings.displayUsdPrice(listing.co), inline: true },
      { name: 'New', value: amount, inline: true },
    ]);
    return interaction.editReply(`C/O for **${listings.displayIgn(listing)}** set to **${amount}**.`);
  }
  ...
  if (sub === 'clear') {
    ...
    await listings.renderPreview(interaction.client, updated);
    sync.emitListingUpdate(updated);
    logs.listing(interaction.client, 'C/O cleared', listing, interaction.user.id, [
      { name: 'Old', value: listings.displayUsdPrice(listing.co), inline: true },
    ]);
    return interaction.editReply(`C/O for **${listings.displayIgn(listing)}** reset to **Offer**.`);
  }
...
async function handleTicket(interaction) {
  ...
  await channel.send({
    embeds: [
      new EmbedBuilder()
        .setColor(config.EMBED_COLOR)
        .setTitle('Finish importing this proxy')
        .setDescription(
