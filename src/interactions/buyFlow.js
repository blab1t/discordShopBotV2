...
async function unavailable(interaction) {
  return interaction
    .reply({ content: 'This listing is not available anymore.', flags: EPH })
    .catch(() => {});
}

function existingTicketReply(interaction, ticket) {
  return interaction.reply({
    content: `You already have an open ticket for this listing: <#${ticket.channel_id}>`,
    flags: EPH,
  });
}
...
// Adds the listing card for a proxy/offer/BIN that is joining a ticket which is
// already open for something else, so the channel says what was just added.
...
function offerReviewEmbed(listing, ticket, reviewedById = null) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Listing: <#${listing.listing_channel_id}>\n` +
      `Offer: **${listings.displayUsdPrice(ticket.offer_amount)}**\n` +
      `Buyer: <@${ticket.creator_id}>\n` +
      `Status: **${state}**${reviewedById ? `\nReviewed by: <@${reviewedById}>` : ''}`
    );
  // While still pending, warn staff if this offer is not above the current C/O.
  if (ticket.offer_status === 'pending') {
    const currentCo = listings.usdToNumber(listing.co);
    const offerValue = listings.usdToNumber(ticket.offer_amount);
    if (currentCo !== null && offerValue !== null && offerValue <= currentCo) {
      embed.addFields({
        name: '⚠️ Below current C/O',
        value: `This offer (**${listings.displayUsdPrice(ticket.offer_amount)}**) is not higher than the current C/O (**${listings.displayUsdPrice(listing.co)}**). Only accept it if the previous offer was withdrawn.`,
        inline: false,
      });
    }
  }
...
async function notifyOwnerOfAcceptedOffer(interaction, listing, amount) {
  try {
    ...
    await owner.send(
      `A staff-approved offer was accepted on your listing **${listings.displayIgn(listing)}**: **${amount}**\n${listingLink}`
    );
    return true;
  } catch (err) {
    return false;
...
async function placeOffer(interaction, listing, amount, choice) {
  ...
  // The buyer's card carries Offer again from the start, so the control is
  // always in the ticket instead of only appearing once staff review the bid.
  ...
  const belowNote = currentCo !== null && offerValue !== null && offerValue <= currentCo
    ? ` Heads up: this is not above the current C/O of **${listings.displayUsdPrice(listing.co)}**, so staff may hold it.`
    : '';
...
async function placeBin(interaction, listing, choice) {
  ...
  const lines = [
    `Listing: <#${listing.listing_channel_id}>`,
    `BIN price: **${listings.displayUsdPrice(listing.bin)}**`,
    `Buyer: <@${interaction.user.id}>`,
  ];
  const title = `BIN purchase for ${listings.displayIgn(listing)}`;
  let channel = target.channel;
  if (target.ticket) {
...
async function handle(interaction, parts) {
  ...
  if (action === 'watch') {
    ...
    await interaction.reply({
      content: watching
        ...
        : `🔔 You will be DMed when the offer or BIN of **${listings.displayIgn(listing)}** changes, or when it sells. Press again to stop.`,
      flags: EPH,
    }).catch(() => {});
  ...
  // Marking sold is destructive-ish (it pulls the listing off the board), so it
  ...
  if (action === 'sold') {
    if (interaction.user.id !== listing.requester_id && !isStaffOrHigher(interaction.member, interaction.guild)) {
      return interaction.reply({ content: 'Only the account owner (proxier) or staff can mark this listing as sold.', flags: EPH });
    }
    return interaction.reply({
      content: `Mark **${listings.displayIgn(listing)}** as sold?\nIt moves to the sold category and loses its Offer/BIN buttons.`,
      components: [
        new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`ls:soldyes:${listing.id}`).setLabel('Yes, mark it sold').setStyle(ButtonStyle.Danger).setEmoji('💰'),
  ...
  if (action === 'soldyes') {
    ...
    await interaction.update({ content: 'Marking it sold...', components: [] }).catch(() => {});
    ...
    require('../services/logs').listing(interaction.client, 'sold', listing, interaction.user.id, [
      { name: 'BIN', value: listings.displayUsdPrice(listing.bin), inline: true },
      { name: 'C/O', value: listings.displayUsdPrice(listing.co), inline: true },
    ]);
    ...
    await listings.notifyWatchers(interaction.client, updated, 'has been marked **SOLD**.').catch(() => {});

    // The proxy owner is pinged first, then staff.
    const mentionParts = [];
    const users = [];
    const roles = [];
    ...
    await noticeChannel.send({
      content: `${mentionParts.join(' ')} the account **${listings.displayIgn(listing)}**${where} was marked sold by <@${interaction.user.id}>.`,
      allowedMentions: { users, roles },
    }).catch(() => {});
    return interaction.editReply({
      content: `**${listings.displayIgn(listing)}** is marked sold and staff were notified.`,
      components: [],
    }).catch(() => {});
  }
  ...
  if (action === 'offerm') {
    ...
    // An offer can join a ticket the buyer already has open, so a support or
    // proxy ticket can carry it instead of spawning another channel.
    const open = db.openTicketsForUser(interaction.user.id);
    if (!open.length) return placeOffer(interaction, listing, amount, 'new');
    return interaction.editReply({
  ...
  // "Offer again" from inside a ticket: no picker, the offer lands in the
  // ticket the button was pressed in.
  if (action === 'again' || action === 'againm') {
    ...
    if (String(ticket.creator_id) !== interaction.user.id) {
      return interaction.reply({
        content: 'Only the buyer this ticket belongs to can offer here. Staff can use `/offer add`.',
        flags: EPH,
      });
    }
    ...
    await interaction.deferReply({ flags: EPH });
    return placeOffer(interaction, listing, amount, String(ticket.id));
  }
  ...
  if (action === 'offerwhere') {
    ...
    return placeOffer(interaction, listing, amount, interaction.values[0]);
  }
  ...
  if (action === 'oaccept' || action === 'odecline' || action === 'offeraccept' || action === 'offerdecline') {
    if (!isStaffOrHigher(interaction.member, interaction.guild)) {
      return interaction.reply({ content: 'Only staff can review offers.', flags: EPH });
    }
    ...
    await interaction.channel.send({
      content: `<@${ticket.creator_id}> Your offer of **${listings.displayUsdPrice(ticket.offer_amount)}** was ${accepted ? 'accepted and set as the C/O' : 'declined'} by <@${interaction.user.id}>.`,
    ...
    if (accepted) {
      ...
      // Also tell the owner in their own proxy ticket, so it is on record even
        ...
        if (ownerTicket) {
          await ownerTicket.send({
            ...
            embeds: [new EmbedBuilder()
              ...
              .setDescription(
                `An offer of **${listings.displayUsdPrice(ticket.offer_amount)}** on **${listings.displayIgn(listing)}** was accepted by <@${interaction.user.id}>.` +
      ...
      await listings.announceListingUpdate(interaction.client, fresh, `Current offer: **${listings.displayUsdPrice(ticket.offer_amount)}**`);
      ...
      // Everyone who bid lower is told they are no longer top, without naming
      // the new bidder.
  ...
  if (action === 'bin') {
    ...
    return interaction.editReply({
      content: `Where should the BIN purchase of **${listings.displayIgn(listing)}** go?`,
