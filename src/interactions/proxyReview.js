...
async function missing(interaction) {
  return interaction.reply({ content: 'That listing no longer exists.', flags: EPH }).catch(() => {});
}
...
async function sendAcceptedNotice(client, listing, staffId, listingChannelId) {
  ...
  await ticketChannel.send({
    content: `✅ Proxy accepted by <@${staffId}>.${listingChannelId ? ` Listing channel: <#${listingChannelId}>.` : ''}`,
...
// Editing actions the account owner may run on their own listing while it is
// still unpublished. Accepting, denying, publishing and deleting stay staff-only.
...
async function handle(interaction, parts) {
  ...
  // The owner may fix their own listing only while it is still under review.
  ...
  if (action === 'accept') {
    ...
    if (listing.status === 'pending' && listing.listing_channel_id) {
      ...
      if (!channel || channel.type !== ChannelType.GuildText) {
        return interaction.followUp({ content: 'The transferred proxy channel no longer exists.', flags: EPH }).catch(() => {});
      }
      ...
      return interaction.followUp({
        content: `Transferred listing approved and published in <#${channel.id}>.`,
        flags: EPH,
      }).catch(() => {});
    }
    ...
    if (listing.listing_channel_id) {
      ...
      return interaction.editReply(`This listing was stuck half-accepted, so I finished it: published in <#${existing.id}>.`);
    }
  ...
  if (action === 'chan') {
    ...
    await channel.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            'Write your content for this listing (images, description, whatever you like). ' +
            'When you are done, press Finish and the listing embed gets posted below.'
          ),
      ],
      components: [
        new ActionRowBuilder().addComponents(
    ...
    return interaction.editReply({
      content: `Listing channel created: <#${channel.id}>. Write your content there, then press Finish.`,
    });
  }
  ...
  if (action === 'finish') {
    ...
    if (listing.listing_message_id) {
      ...
      if (posted) {
        return interaction.reply({ content: 'This listing is already published.', flags: EPH });
      }
    ...
    const channel = await interaction.client.channels.fetch(listing.listing_channel_id).catch(() => null);
    if (!channel) {
      return interaction.followUp({ content: 'The listing channel no longer exists.', flags: EPH }).catch(() => {});
    }
    const published = await channel.send(listings.listingPayload({ ...listing, status: 'published' }, 'published'));
    ...
    await interaction.followUp({ content: 'Listing published. ✅', flags: EPH }).catch(() => {});
  ...
  if (action === 'deny') {
    ...
    await interaction.reply({ content: 'Listing denied.', flags: EPH });
    await rerender(interaction.client, listing.id);
    ...
    if (ticketChannel) {
      await ticketChannel.send({
        ...
        embeds: [
          new EmbedBuilder()
            .setColor(0xed4245)
            .setDescription(`Your proxy request for **${listings.displayIgn(listing)}** was denied by <@${interaction.user.id}>.`),
        ],
  ...
  if (action === 'editinfo') {
    ...
    await rerender(interaction.client, listing.id);
    return interaction.editReply({ content: 'General information updated.' });
  }
  ...
  if (action === 'editprice') {
    ...
    await rerender(interaction.client, listing.id);
    return interaction.editReply({ content: 'Prices updated.' });
  }
  ...
  if (action === 'editmeta') {
    ...
    if (mojang.isHiddenPlaceholder(ign)) {
      return interaction.reply({
        content: '⚠️ That field needs the real Minecraft username. To keep it private use `/proxy hide` (or the Hide username toggle) instead - the card then shows "Hidden" while staff still see the real name.',
        flags: EPH,
      });
    }
    if (!mojang.isValidIgn(ign)) {
      return interaction.reply({ content: 'That is not a valid Minecraft username.', flags: EPH });
    }
    if (!category) {
      return interaction.reply({
    ...
    if (ign.toLowerCase() !== listing.ign.toLowerCase()) {
      ...
      const resolved = await mojang.resolveUser(ign).catch(() => {
        lookupFailed = true; // outage: keep the edit rather than blocking staff
        return null;
      });
      if (!resolved && !lookupFailed) {
        return interaction.editReply(`⚠️ **${ign}** is not an existing Minecraft account. Check the spelling; use the hide option to keep a real name private.`);
      }
    ...
    await rerender(interaction.client, listing.id);
    return interaction.editReply({ content: 'Username and category updated.' });
  }
  ...
  if (action === 'delyes') {
    ...
    if (listing.listing_channel_id) {
      ...
      const channel = await interaction.client.channels.fetch(listing.listing_channel_id).catch(() => null);
      if (channel) await channel.delete('Proxy listing deleted').catch(() => {});
    }
    ...
    return interaction.editReply({
      content: `Listing **${listings.displayIgn(listing)}** deleted${localOnly ? ' on this server only.' : '.'}`,
      components: [],
    });
  }
