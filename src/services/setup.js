...
function buildProxyPanel() {
  const embed = new EmbedBuilder()
    ...
    .setTitle('Create a Proxy Listing')
    .setDescription(
      'Want us to list your account? Click the button below.\n\n' +
      'Pick a category, enter the IGN, confirm the detected capes and fill in the account details. ' +
      'Staff reviews every listing before it goes live.'
    );
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('panel:proxy').setLabel('Create Proxy').setStyle(ButtonStyle.Primary).setEmoji('📋')
  );
...
function buildTicketPanel() {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      '🛒 **Buy an Account** - purchase one of our listings\n' +
      '📋 **Proxy an Account** - get your account listed\n' +
      '❓ **Other** - anything else'
    );
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('tp:buy').setLabel('Buy an Account').setStyle(ButtonStyle.Success).setEmoji('🛒'),
    new ButtonBuilder().setCustomId('tp:proxy').setLabel('Proxy an Account').setStyle(ButtonStyle.Primary).setEmoji('📋'),
...
module.exports = {
  runSetup, ensureVouchPermissions, arrangeListingCategories, buildProxyPanel, buildTicketPanel, buildVerificationMessage, ensureListingCategory, removeListingCategory, ensureSoldCategory,
